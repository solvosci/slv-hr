...
class HrLeave(models.Model):
    ...
    def activity_update(self):
        ...
        for holiday in self.filtered(lambda x: x.holiday_status_id.leave_validation_type == 'both'):
            ...
            responsibles = holiday.holiday_status_id.responsible_ids
    ...
    def message_subscribe(self, partner_ids=None, subtype_ids=None):
        if self.holiday_status_id.leave_validation_type == 'both':
            responsibles = self.holiday_status_id.responsible_ids
            for responsible in responsibles.filtered(lambda x: x.partner_id and x.partner_id.id not in partner_ids):
                partner_ids.append(responsible.partner_id.id)
        return super(HrLeave, self).message_subscribe(partner_ids=partner_ids, subtype_ids=subtype_ids)
